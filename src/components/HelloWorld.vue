...
export default {
  ...
  methods: {
    ...
    getPie () {
      let pieData = this.genPieData(50)
      let pie = {
        title: {
          text: '同名数量统计',
          subtext: '纯属虚构'
    ...
    convertChinaMapData (data) {
      let res = []
      for (let i = 0; i < data.length; i++) {
        let geoCoord = this.chinaMapData[data[i].name]
        if (geoCoord) {
          res.push({
            name: data[i].name,
